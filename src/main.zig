...
const LARGEST_BOARD_SIDE = 9;
const LARGEST_BOARD = 9 * 9;
...
const TileTag = enum { mine, value };
const TileType = union(TileTag) { mine: void, value: u8 };
const Tile = struct {
    type: TileType,
    flag: bool,
    uncovered: bool,
...
const DifficultyTag = enum {
    easy,
    medium,
    hard,
};
const Difficulty = union(DifficultyTag) {
    easy: void,
    medium: void,
    hard: void,
    ...
    return struct {
        ...
        pub fn uncover(self: *@This(), x: usize, y: usize) void {
            ...
            while (0 < len) {
                ...
                if (tile.uncovered == true) continue;
                switch (tile.type) {
                    .mine => return,
                    .value => |value| {
                        tile.uncovered = true;
...
pub fn main() !void {
    var board = Board(LARGEST_BOARD_SIDE).init(Difficulty.easy) orelse unreachable;
    board.uncover(1, 1);
    board.draw();
}
